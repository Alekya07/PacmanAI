    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            new_dist = self.getPositionDistribution(self.setGhostPosition(gameState,oldPos))
            for new_pos,prob in new_dist.items():
                allPossible[new_pos]+=prob*self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance==None:
            ...
            for loc in self.legalPositions:
                ...
                allPossible[loc]= allPossible[loc] + (emissionModel[dist]*old_b[loc])
            ...
            if not any(allPossible.values()):
                ...
                for num in range(self.numParticles):
                    par.append(util.sample(allPossible))
                self.particles=par
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        #new_pos=self.getPositionDistribution(self.setGhostPosition(gameState, x)), self.particle)
        #self.particle=map(util.sample(new_pos), new_pos)
        temp=[]
        for old_pos in self.particles:
            new_pos = self.getPositionDistribution(self.setGhostPosition(gameState,old_pos))
            temp.append(util.sample(new_pos))
        self.particles=temp
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(len(oldParticle)):
                new_dist=getPositionDistributionForGhost(
                    setGhostPositions(gameState,oldParticle),i,self.ghostAgents[i]
                )
                newParticle[i]=util.sample(new_dist)
